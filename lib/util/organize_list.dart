...
List<Reminder> organizeByColor(List<Reminder> currentList){
  ...
  for(int i = 0; i < currentList.length; i++){
    ...
    while(currentIndex < currentList.length && !addedReminder){ 
      ...
      if(reminderInSorted.cardColor == currentReminder.cardColor){
        ...
        addedReminder = true;
      }else if(currentIndex == currentList.length - 1){ //Looped through all the elements but still wasn't able to find a match. Just add to the end of the array as a result
        sortedReminders.add(currentReminder);
      }
